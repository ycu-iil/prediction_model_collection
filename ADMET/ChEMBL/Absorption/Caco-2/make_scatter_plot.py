...
def make_scatter_plot(y_true, y_pred):
    ...
    ax.set_xlabel('Predicted value', fontsize=20, labelpad=20, weight='normal')
    ax.set_ylabel('Experimental value', fontsize=20, labelpad=20, weight='normal')
...
def main():
    # Data preparation
    suppl = Chem.SDMolSupplier('./data/permeability_AB_ChEMBL.sdf')
    mols = []
    values = []
    for mol in suppl:
        value = float(mol.GetProp('Permeability'))
        if isinf(value):
            continue
        mols.append(mol)
        values.append(value)
