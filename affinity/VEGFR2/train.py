...
def get_parser():
    parser = argparse.ArgumentParser(
        usage=f'ex.) python {os.path.basename(__file__)} -o lgb_vegfr2'
    )
    parser.add_argument(
        '-o', '--output', type=str, required=False, default='lgb_vegfr2',
...
def main():
    ...
    for mol in suppl:
        if mol is None:
            continue
        try:
            value = mol.GetProp('pCV_Vascular endothelial growth factor receptor 2')
        except:
            continue
        mols.append(mol)
