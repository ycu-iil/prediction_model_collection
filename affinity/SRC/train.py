...
def get_parser():
    parser = argparse.ArgumentParser(
        usage=f'ex.) python {os.path.basename(__file__)} -o lgb_src'
    )
    parser.add_argument(
        '-o', '--output', type=str, required=False, default='lgb_src',
...
def main():
    ...
    for mol in suppl:
        if mol is None:
            continue
        try:
            value = mol.GetProp('pCV_Tyrosine-protein kinase SRC')
        except:
            continue
        mols.append(mol)
