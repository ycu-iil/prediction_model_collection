...
def get_parser():
    parser = argparse.ArgumentParser(
        usage=f'ex.) python {os.path.basename(__file__)} -o lgb_lck'
    )
    parser.add_argument(
        '-o', '--output', type=str, required=False, default='lgb_lck',
...
def main():
    ...
    for mol in suppl:
        if mol is None:
            continue
        try:
            value = mol.GetProp('pCV_Tyrosine-protein kinase LCK')
        except:
            continue
        mols.append(mol)
