...
def get_parser():
    parser = argparse.ArgumentParser(
        usage=f'ex.) python {os.path.basename(__file__)} -o lgb_hck'
    )
    parser.add_argument(
        '-o', '--output', type=str, required=False, default='lgb_hck',
...
def main():
    ...
    for mol in suppl:
        if mol is None:
            continue
        try:
            value = mol.GetProp('pCV_Tyrosine-protein kinase HCK')
        except:
            continue
        mols.append(mol)
