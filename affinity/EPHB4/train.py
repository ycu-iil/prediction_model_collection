...
def get_parser():
    parser = argparse.ArgumentParser(
        usage=f'ex.) python {os.path.basename(__file__)} -o lgb_ephb4'
    )
    parser.add_argument(
        '-o', '--output', type=str, required=False, default='lgb_ephb4',
...
def main():
    ...
    for mol in suppl:
        if mol is None:
            continue
        try:
            value = mol.GetProp('pCV_Ephrin type-B receptor 4')
        except:
            continue
        mols.append(mol)
